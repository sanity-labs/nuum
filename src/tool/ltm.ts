 ...
 */
export function renderCompactTree(entries: LTMEntry[], displayDepth: number): string {
  if (entries.length === 0) {
    return "(empty)"
  }
...
Example: ltm_glob({ pattern: "/knowledge/**" })`,
  ...
  parameters: z.object({
    ...
    maxDepth: z.number().optional().describe("Override display depth calculation"),
  }),
...
Example: ltm_create({
  ...
  parameters: z.object({
    ...
    links: z.array(z.string()).optional().describe("Explicit links to other entries"),
  }),
