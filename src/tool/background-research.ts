...
export const BackgroundResearchTool = Tool.define<
  typeof parameters,
...
>('background_research', {
  ...
  async execute({topic}, ctx) {
    ...
    const storage = (
      ctx as Tool.Context & {extra: BackgroundResearchToolContext}
    ).extra?.storage
    ...
    log.info('spawning background research', {
      taskId,
...
async function runBackgroundResearch(
  ...
  try {
    ...
    const report = [
      ...
      entriesCreated > 0
        ? `*Created ${entriesCreated} LTM entries: ${result.entriesCreated.join(', ')}*`
        : '',
      entriesUpdated > 0
        ? `*Updated ${entriesUpdated} LTM entries: ${result.entriesUpdated.join(', ')}*`
        : '',
    ]
      .filter(Boolean)
      .join('\n')
