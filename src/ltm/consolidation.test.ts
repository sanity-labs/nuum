...
function createTempDb(): string {
  ...
  return path.join(
    tmpDir,
    `test-consolidation-${Date.now()}-${Math.random().toString(36).slice(2)}.db`,
  )
}
...
describe('isConversationNoteworthy', () => {
  ...
  it('returns true for conversations with tool usage', () => {
    const messages: TemporalMessage[] = [
      ...
      },
      {
        id: '3',
        type: 'tool_call',
        content: '{"name":"read","args":{"path":"README.md"}}',
        tokenEstimate: 10,
        createdAt: new Date().toISOString(),
      },
      {
      ...
      },
      {
        id: '5',
        type: 'assistant',
        content: "Here's what the README says...",
        tokenEstimate: 50,
        createdAt: new Date().toISOString(),
      },
    ]
  ...
  it('returns true for conversations with substantial content', () => {
    const longContent =
      'This is a detailed explanation about how the authentication system works. '.repeat(
        10,
      )
    const messages: TemporalMessage[] = [
      {
        id: '1',
        type: 'user',
        content: 'Explain the auth system',
        tokenEstimate: 5,
        createdAt: new Date().toISOString(),
      },
      {
...
describe('LTM storage integration', () => {
  ...
  describe('reparent', () => {
    it('moves entry to new parent', async () => {
      ...
      const updated = await storage.ltm.reparent(
        'child',
        'parent-b',
        1,
        'ltm-consolidate',
  ...
  describe('rename', () => {
    it('changes entry slug and path', async () => {
      ...
      const updated = await storage.ltm.rename(
        'old-name',
        'new-name',
        1,
        'ltm-consolidate',
