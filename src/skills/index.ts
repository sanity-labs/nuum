...
export function formatSkillsCatalog(skills: Skill[]): string | null {
  ...
  lines.push('')
  lines.push(
    "**When to use:** If a task matches a skill's description, use that skill. The user may also request a specific skill by name.",
  )
  lines.push('')
  lines.push('**How to use (progressive disclosure):**')
  lines.push(
    "1. Read the skill's `SKILL.md` file - only what you need for the current task",
  )
  lines.push(
    '2. If it references `scripts/`, `references/`, or `assets/`, load only the specific files needed',
  )
  lines.push('3. Prefer running existing scripts over rewriting code')
  lines.push('')
  lines.push(
    "**Multiple skills:** Choose the minimal set that covers the request. State which skills you're using and why.",
  )
  lines.push('')
  lines.push(
    "**Context hygiene:** Summarize long sections instead of pasting them. Don't bulk-load reference files.",
  )
  lines.push('')
  lines.push(
    "**Fallback:** If a skill can't be applied (missing files, unclear instructions), state the issue and continue with your best approach.",
  )
