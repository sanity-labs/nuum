 ...
 */
export function createBackgroundStorage(db: DrizzleDB | AnyDrizzleDB): BackgroundStorage {
  return {
    ...
    async getUnsurfaced(): Promise<BackgroundReport[]> {
      ...
      return rows.map((row: { id: string; createdAt: string; subsystem: string; report: string; surfacedAt: string | null }) => ({
        id: row.id,
